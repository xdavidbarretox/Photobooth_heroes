...
public class PhotoBoothController : MonoBehaviour, KinectGestures.GestureListenerInterface, InteractionListenerInterface
{
    ...
    [Tooltip("Reference to the right hand joint-overlayer component.")]
    public JointOverlayer rightHandOverlayer;
    ...
    [Tooltip("Array of sprite transforms that will be used for right hand overlays on each step.")]
    public Transform[] rightHandMasks;
	...
	void Start () 
	{
        ...
        if (rightHandMasks != null && rightHandMasks.Length > maskCount)
            maskCount = rightHandMasks.Length;
	...
	void Update () 
	{
		...
		if (currentIndex != prevIndex) 
		{
            ...
            if (rightHandOverlayer && rightHandMasks != null)
            {
                if (rightHandOverlayer.overlayObject)
                {
                    rightHandOverlayer.overlayObject.rotation = rightHandOverlayer.initialRotation;
                    rightHandOverlayer.overlayObject.gameObject.SetActive(false);
                }

                rightHandOverlayer.overlayObject = currentIndex >= 0 && currentIndex < rightHandMasks.Length ? rightHandMasks[currentIndex] : null;
                rightHandOverlayer.playerIndex = playerIndex;

                if (rightHandOverlayer.overlayObject)
                {
                    rightHandOverlayer.overlayObject.gameObject.SetActive(true);
                }

                rightHandOverlayer.Start();
            }
